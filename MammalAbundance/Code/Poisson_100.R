...
#------Function to package and run Poisson 500m model-------

dm500p <- function(y, n.burn = 1, n.it = 3000, n.thin = 1, outfile = "Diagnostic_Plots.pdf", outfile2 = "Table.csv") {
  force(n.burn); force(n.it); force(n.thin)
  Name <- y
  nplots <- nrow(y)
  ...
  jdata100 <- list(y=y, nplots=nplots, ncap=ncap, 
                   ...
                   #wet.100 = Landscape.s[ , "Wetland_100"]
                   # dist.wet = Landscape.s[, "D_wetland_buff"],
                   age.100 = Landscape.s[ , "Age_500"],
                   hard.100 = Landscape.s[ , "H_500"],
                   mix.100 = Landscape.s[ , "M_500"],
                   stream.100 = Landscape.s[ , "Stream_500"],
                   #day=as.matrix(day.s),
                   nsites = length(unique(Habitat$Site)),
                   nyears=3)
